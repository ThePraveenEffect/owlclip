...
const plans = [
  {
    ...
    price: 9,
    period: '/mo',
    description: 'Start turning long videos into short viral clips.',
    credits: 250,
    highlight: '~25 video processing sessions',
    features: [
  ...
  },
  {
    ...
    price: 29,
    period: '/mo',
    description: 'Built for creators publishing consistently.',
    credits: 1000,
    highlight: '~100 video processing sessions',
    features: [
...
export default function BillingSection() {
  ...
  // Helper utility to inject current local authorization headers safely
  const getAuthHeaders = () => {
    const token = localStorage.getItem('access_token'); // Ensure this matches your storage key exactly
    return {
      headers: {
  ...
  const verifyUserPayment = async () => {
    try {
      const res = await axios.get(`${env.BASE_URL}/v1/payment/verify`, getAuthHeaders());
      ...
      if (error.response?.status === 401) {
        // Soft fail on initial mounting verification so it doesn't break rendering
  ...
  const handleBuyClick = async (type: string) => {
    try {
      const order = await axios.post(
        `${env.BASE_URL}/v1/payment/create`,
        { tier: type },
        getAuthHeaders()
      );
      ...
      const options = {
        ...
        handler: async (response: any) => {
          // Fire verification immediately after Razorpay modal completes transaction
      ...
      if (error.response?.status === 401) {
        alert("Your session has expired. Please log back in to upgrade.");
        router.push('/login');
      } else {
        alert("Something went wrong initializing your checkout. Please try again.");
      }
    }
  };
